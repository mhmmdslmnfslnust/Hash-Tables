...
int main() {
    ...
    ofstream fout("hash_performance.csv");
    fout << "N,Direct Addressing,Linear Probing,Quadratic Probing,Double Hashing\n";
    ...
    for (int N = 1; N <= 997; ++N) {
        ...
        fout << N << ","
             << total_da / repetitions << ","
             << total_lp / repetitions << ","
             << total_qp / repetitions << ","
             << total_dh / repetitions << "\n";
    }

    fout.close();
    cout << "Data written to hash_performance.csv" << endl;
    return 0;
}
